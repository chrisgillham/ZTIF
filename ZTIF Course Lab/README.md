Zero Trust Input Validation with AI-Augmented Semantic Analysis | Lab Assignment 3
...
1. Articulate why structural OWASP validation alone is insufficient in Zero Trust and AI-integrated architectures.
2. Author a formal Zero Trust Intent Contract in YAML that defines the semantic boundaries of an input surface.
3. Build and deploy a 4-gate input validation pipeline integrating OWASP controls, ZT policy enforcement, and LLM semantic analysis.
4. Integrate the LLM guard verdict with a Discord HITL (Human-in-the-Loop) notification channel.
5. Evaluate validation pipeline coverage against OWASP LLM Top 10 and MITRE ATLAS threat categories.
6. Write defensible test cases (pass, flag, block) that serve as executable security controls.
...
> ⚠️ **KEY INSIGHT:** A well-formed payload that violates semantic intent — e.g., sending "ignore previous instructions" to a product search field — is invisible to regex and schema validators. The LLM guard operates at the meaning layer where traditional validators cannot reach.

Resources & References
Required Reading:
* OWASP Input Validation Cheat Sheet — owasp.org/cheat-sheets
...
* ZTIF-IC-001 Schema Reference — github.com/ztif/ztif-intent-contracts
